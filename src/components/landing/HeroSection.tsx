          ...
          >
            ...
            <div className="flex items-center gap-6 text-sm text-muted-foreground">
              <div className="flex -space-x-2">
                {["🎂", "🎁", "💐", "🎉"].map((emoji, i) => (
                  <div key={i} className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-lg border-2 border-background">
                    {emoji}
                  </div>
                ))}
              </div>
              <span>Join <strong className="text-foreground">10,000+</strong> happy gifters</span>
            </div>
          </motion.div>
