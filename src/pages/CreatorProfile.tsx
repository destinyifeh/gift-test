...
const enabledUsers: Record<string, {
  ...
  destiny: {
    ...
    totalReceived: 320, totalSupporters: 28,
  },
};
        ...
        <div className="pt-24 flex items-center justify-center px-4">
          <Card className="max-w-md w-full border-border shadow-elevated">
            <CardContent className="p-8 text-center">
              ...
              <h1 className="text-xl font-bold font-display text-foreground mb-2">@{username}</h1>
              <p className="text-muted-foreground mb-6">This user has not enabled gifts yet.</p>
              <Badge variant="outline" className="text-sm">Gift page inactive</Badge>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="pt-20 pb-16">
        <div className="container mx-auto px-4 max-w-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-foreground mt-3 max-w-md mx-auto">{profile.bio}</p>
            <div className="flex items-center justify-center gap-4 mt-4 text-sm text-muted-foreground">
              <span className="flex items-center gap-1"><Heart className="w-4 h-4 text-primary" /> {profile.totalSupporters} supporters</span>
              <span className="flex items-center gap-1"><Gift className="w-4 h-4 text-secondary" /> ${profile.totalReceived} received</span>
            </div>
          </div>
            ...
            <CardContent className="p-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {profile.suggestedAmounts.map((a) => (
                  <Button key={a} variant="outline" className="h-14 text-lg font-bold" onClick={() => setShowGiftModal(true)}>${a}</Button>
                ))}
              </div>
          ...
          {profile.showSupporters && (
            <Card className="border-border">
              <CardContent className="p-6">
                <h3 className="font-semibold text-foreground mb-4 flex items-center gap-2"><Heart className="w-4 h-4 text-primary" /> Supporters</h3>
                <div className="space-y-4">
                  {profile.supporters.map((s) => (
                    <div key={s.id} className="flex items-start gap-3">
                      <Avatar className="w-8 h-8"><AvatarFallback className="bg-muted text-xs">{s.anonymous ? "?" : s.name.charAt(0)}</AvatarFallback></Avatar>
