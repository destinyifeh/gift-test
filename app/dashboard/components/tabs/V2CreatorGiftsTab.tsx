...
export function V2CreatorGiftsTab({setSection, setWalletView}: V2CreatorGiftsTabProps) {
  ...
  const {data: creatorRes, isLoading} = useCreatorSupporters(page);

  const creatorGiftsList = creatorRes?.data || [];

  // Calculate stats
  const totalReceivedCount = creatorRes?.total || creatorGiftsList.length;
  const totalValue = creatorRes?.totalReceived || creatorGiftsList.reduce((sum: number, g: any) => sum + (g.amount || 0), 0);
