...
export function GiftCard3D({ 
  ...
  const getContainerClasses = () => {
      const base = "absolute inset-0 w-full h-full rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-10 flex flex-col justify-between overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] [backface-visibility:hidden] border border-white/20";
  ...
  return (
    ...
    >
            ...
            <div className="flex justify-between items-start relative z-10 w-full">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <div className="font-headline font-black text-white tracking-tight text-xl md:text-3xl leading-none truncate max-w-[140px] md:max-w-[240px]">
                            Gifthance
                        </div>
                        <div className="text-[9px] md:text-[11px] font-bold text-white/60 uppercase tracking-[0.1em] md:tracking-[0.2em] mt-1 md:mt-2 truncate">
                            {vendorName ? `${vendorName} ${cardName || 'Gift Card'}` : (cardName || 'Gift Card')}
                        </div>
                    </div>
                </div>
                <div className="px-3 py-1 md:px-4 md:py-2 bg-white/20 border border-white/10 rounded-full flex items-center justify-center -mr-2 md:mr-0 mt-1 md:mt-2 min-w-[36px] md:min-w-[40px]">
                    {icon ? (
                         <span className="v2-icon text-white text-base md:text-xl font-bold">{icon}</span>
                    ) : (
                         <span className="text-white text-[10px] md:text-sm font-black uppercase tracking-widest leading-none">Active</span>
                    )}
                </div>
            </div>
            <div className="relative z-10 flex flex-col justify-center mt-8 md:mt-10 pl-1 md:pl-2">
                <p className="text-white/70 text-[8px] md:text-xs font-black uppercase tracking-[0.1em] mb-0.5 md:mb-2">Available Balance</p>
                <p className="text-white font-black text-[1.8rem] md:text-[3.5rem] tracking-tight leading-none">
                    {amount && amount > 0 ? `₦${amount.toLocaleString()}` : "₦---"}
                </p>
            </div>

            <div className="flex justify-end items-end relative z-10 border-t border-white/10 pt-2.5 md:pt-8 mt-auto pb-0">
                <div 
        ...
        >
            ...
            <div className="flex justify-between items-end relative z-10 border-t border-white/10 pt-2.5 md:pt-8 mt-auto pb-0">
