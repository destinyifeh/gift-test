...
import { useAdminCreatorGifts, useFlagCreatorGift } from '@/hooks/use-admin';
...
export function V2AdminCreatorGiftsTab({
  ...
  const { data: infiniteData, isLoading } = useAdminCreatorGifts({ search: searchQuery });
  ...
  const flagMutation = useFlagCreatorGift();
  ...
  const confirmFlag = () => {
    ...
    flagMutation.mutate({ 
      id: flagModal.gift.id, 
      reason: flagReason,
      action: 'flag'
    }, {
      onSuccess: () => {
        addLog(`Flagged gift ${flagModal.gift.id.slice(0, 8)}… — Reason: "${flagReason}"`);
