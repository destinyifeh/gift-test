...
import { useAdminWallets, useUpdateWalletStatus } from '@/hooks/use-admin';
...
export function V2AdminWalletsTab({
  ...
  const { data: infiniteData, isLoading } = useAdminWallets({ search: searchQuery });
  ...
  const walletMutation = useUpdateWalletStatus();

  const handleWalletUpdate = async (id: string, status: string) => {
    try {
      await walletMutation.mutateAsync({ id, status });
      toast.success(`Wallet ${status === 'frozen' ? 'frozen' : 'unfrozen'} successfully`);
      addLog(`${status === 'frozen' ? 'Froze' : 'Unfroze'} wallet ${id.slice(0, 8)}…`);
    } catch (error) {
  ...
  const filteredWallets = wallets.filter((w: any) => {
    const matchesSearch =
      !localSearch ||
      String(w.user || '').toLowerCase().includes(localSearch.toLowerCase());
  ...
  const confirmAction = () => {
    ...
    handleWalletUpdate(actionModal.wallet.id, newStatus);
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="w-10 h-10 rounded-full bg-[var(--v2-surface-container)] flex items-center justify-center font-bold text-[var(--v2-primary)]">
                      {String(wallet.user || 'Unknown').charAt(0).toUpperCase()}
                    </div>
