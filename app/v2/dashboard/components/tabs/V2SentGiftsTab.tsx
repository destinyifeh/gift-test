...
export function V2SentGiftsTab() {
  ...
  const {data: sentRes, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage} =
    useInfiniteQuery({
      queryKey: ['sent-gifts'],
      initialPageParam: 0,
  ...
  const totalValue = sentGiftsList.reduce((sum, g) => sum + (g.amount || 0), 0);
  const scheduledCount = sentGiftsList.filter(g => g.status === 'scheduled' || g.status === 'claimable').length;

  // Filter gifts
  const filteredGifts =
    filterStatus === 'all'
      ? sentGiftsList
      : sentGiftsList.filter(g => g.status === filterStatus);
