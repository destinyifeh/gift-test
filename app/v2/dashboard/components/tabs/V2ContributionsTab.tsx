...
export function V2ContributionsTab() {
  const {data: contribRes, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage} =
    useInfiniteQuery({
  ...
  const contributionsList = contribRes?.pages.flatMap(p => p.data || []) || [];

  // Calculate stats
  const totalContributed = contributionsList.reduce((sum, c) => sum + (c.amount || 0), 0);
  const campaignsSupported = new Set(contributionsList.map(c => c.campaignId || c.campaignName)).size;
