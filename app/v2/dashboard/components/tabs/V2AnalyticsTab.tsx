...
import {fetchCreatorAnalytics, fetchCreatorSupporters} from '@/lib/server/actions/analytics';
...
export function V2AnalyticsTab({setSection, setWalletView}: V2AnalyticsTabProps) {
  ...
  const {data: analyticsRes, isLoading} = useQuery({
    ...
    queryFn: () => fetchCreatorAnalytics({username}),
    enabled: !!username,
  });

  // Fetch recent supporters for the activity section
  const {data: supportersRes} = useQuery({
    queryKey: ['creator-supporters-recent', username],
    queryFn: () => fetchCreatorSupporters({username, pageParam: 0}),
    enabled: !!username,
  });

  // Type guard to check if data has analytics shape (not array)
  const analyticsData =
    analyticsRes?.data && !Array.isArray(analyticsRes.data) ? analyticsRes.data : null;
    ...
    <div className="space-y-6">
      ...
      <div className="hidden md:flex justify-between items-start">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => {
              navigator.clipboard.writeText(`${window.location.origin}/v2/u/${username}`);
