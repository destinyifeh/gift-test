...
export function V2ReceivedGiftsTab({setSection, setWalletView}: V2ReceivedGiftsTabProps) {
  ...
  const {data: receivedRes, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage} =
    useInfiniteQuery({
  ...
  const totalValue = receivedGiftsList.reduce((sum, g) => sum + (g.amount || 0), 0);
  const recentCount = receivedGiftsList.filter(g => {
  ...
  const filteredGifts =
    filterStatus === 'all'
      ? receivedGiftsList
      : receivedGiftsList.filter(g => g.status === filterStatus);
