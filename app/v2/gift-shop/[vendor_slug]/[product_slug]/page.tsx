  ...
  useEffect(() => {
    const loadRelated = async () => {
      ...
      if (result.success && result.data) {
        ...
        const filtered = result.data.filter((p: any) => p.id !== product.id);
    ...
    <div className="min-h-screen bg-[var(--v2-background)]">
      ...
      <main className="pt-16 md:pt-24 pb-32 md:pb-16">
        ...
        <nav className="hidden md:flex items-center gap-2 mb-8 text-[var(--v2-on-surface-variant)] text-sm px-6 max-w-7xl mx-auto">
          ...
          <Link href={`/v2/gift-shop/${vendor_slug}`} className="hover:text-[var(--v2-primary)] capitalize">
            ...
            <div className="hidden md:grid grid-cols-4 gap-6">
              {relatedProducts.map((relatedProduct) => (
                <Link
                  ...
                  href={`/v2/gift-shop/${relatedProduct.profiles?.shop_slug || relatedProduct.vendor_id}/${relatedProduct.slug || relatedProduct.id}`}
