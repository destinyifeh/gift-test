...
export function V2VendorPromotionsTab({onBoostProduct}: V2VendorPromotionsTabProps) {
  ...
  const loadPromotions = async () => {
    setIsLoading(true);
    const result = await fetchVendorPromotions();
  ...
  const handlePause = async (promotionId: number) => {
    ...
    const result = await pausePromotion(promotionId);
    if (result.success) {
      toast.success('Promotion paused');
      loadPromotions();
    } else {
      toast.error(result.error || 'Failed to pause promotion');
    }
  ...
  const handleResume = async (promotionId: number) => {
    setActionLoading(promotionId);
    const result = await resumePromotion(promotionId);
    if (result.success) {
      toast.success('Promotion resumed');
      loadPromotions();
    } else {
      toast.error(result.error || 'Failed to resume promotion');
    }
    setActionLoading(null);
  };
