...
export default function CreatorProfilePage({params}: {params: Promise<{username: string}>}) {
  ...
  const {data: supportersData} = useInfiniteQuery({
              ...
              <div className="sticky top-28">
                ...
                <div className="bg-[var(--v2-surface-container-lowest)] rounded-[2.5rem] shadow-2xl shadow-black/5 relative overflow-hidden">
                  ...
                  <div className="p-8 pt-0 -mt-14 relative">
                    ...
                    {isOwner && (
                      <Link
                        href="/v2/dashboard?tab=gift-page"
        ...
        {!(isPro && proRemoveBranding) && (
          <div className="text-center mt-12 pb-4">
            <p className="text-xs text-[var(--v2-on-surface-variant)]">
              Powered by{' '}
              <Link href="/v2" className="font-bold text-[var(--v2-primary)] hover:underline">
                Gifthance
              </Link>
            </p>
          </div>
  ...
  return (
    <header className="lg:hidden fixed top-0 w-full z-50 bg-[var(--v2-surface)]/90 backdrop-blur-xl border-b border-[var(--v2-outline-variant)]/10">
      <div className="flex justify-between items-center px-4 h-16">
        <Link href={isOwner ? '/v2/dashboard' : '/v2'} className="flex items-center gap-2">
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      paystack.newTransaction({
        ...
        //currency: currency,
        currency:'NGN', // Paystack only accepts NGN, convert on backend if needed
        onSuccess: async (response: any) => {
          try {
            const res = await recordCreatorGift({
              ...
              currency,
              giftId: null, // Only used for vendor gift cards
