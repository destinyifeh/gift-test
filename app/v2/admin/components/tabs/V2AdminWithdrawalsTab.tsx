...
import {fetchAdminWithdrawals, updateTransactionStatus} from '@/lib/server/actions/admin';
...
export function V2AdminWithdrawalsTab({
  ...
  const {data: infiniteData, isLoading} = useInfiniteQuery({
    ...
    queryFn: ({pageParam = 0}) => fetchAdminWithdrawals({search: searchQuery, pageParam}),
  ...
  const mutation = useMutation({
    ...
    onSuccess: (res: any, vars) => {
      ...
      addLog(`Processed withdrawal: ${vars.status}`);
    },
              ...
              <div key={withdrawal.id} className="p-4">
                ...
                {withdrawal.status === 'pending' && (
                  <div className="flex gap-2 mt-3">
                    <button
                      onClick={() => mutation.mutate({id: withdrawal.id, status: 'completed'})}
                      className="flex-1 py-2 bg-emerald-100 text-emerald-700 rounded-lg text-xs font-bold">
                      Approve
                    </button>
                    <button
                      onClick={() => mutation.mutate({id: withdrawal.id, status: 'rejected'})}
                  ...
                  <td className="px-8 py-4">
                    {withdrawal.status === 'pending' ? (
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            mutation.mutate({id: withdrawal.id, status: 'completed'})
                          }
                        ...
                        <button
                          onClick={() =>
                            mutation.mutate({id: withdrawal.id, status: 'rejected'})
                          }
