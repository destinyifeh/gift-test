...
import {fetchAdminUsers, fetchAdminVendors, updateUserRole, updateVendorStatus} from '@/lib/server/actions/admin';
...
export function V2AdminVendorsTab({
  ...
  // Filter vendors based on local search
  ...
  const {data: usersData, isLoading: usersLoading} = useQuery({
    queryKey: ['admin-users-for-vendor', addVendorSearch],
    queryFn: () => fetchAdminUsers({search: addVendorSearch, pageParam: 0}),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (res, vars) => {
      ...
      toast.success(`Vendor ${vars.status === 'active' ? 'activated' : 'suspended'} successfully`);
  ...
  const addVendorMutation = useMutation({
    mutationFn: ({userId, roles}: {userId: string; roles: string[]}) =>
      updateUserRole({userId, roles, adminRole: null}),
    onSuccess: (res, vars) => {
      if (!res.success) {
      ...
      queryClient.invalidateQueries({queryKey: ['admin-vendors']});
      queryClient.invalidateQueries({queryKey: ['admin-users-for-vendor']});
      toast.success('Vendor added successfully');
      addLog(`Added vendor role to user ${vars.userId}`);
