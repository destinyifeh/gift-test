...
import {fetchAdminUsers, updateUserSystemStatus} from '@/lib/server/actions/admin';
...
export function V2AdminUsersTab({
  ...
  const users = allUsers.filter((user: any) => {
    ...
    else if (filterType === 'admin') filterMatch = user.roles?.includes('admin') || !!user.admin_role;
    else if (filterType === 'vendor') filterMatch = user.roles?.includes('vendor');
    else if (filterType === 'creator') filterMatch = user.is_creator;
  ...
  const confirmAction = () => {
    ...
    if (actionModal.type === 'activate') {
      ...
      mutation.mutate({
        id: actionModal.user.id,
        updates: {status: 'suspended', suspension_end: suspensionEnd},
      });
